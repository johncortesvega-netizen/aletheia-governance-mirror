...
APP_NAVIGATION_MAP = [
    ...
    ("Receipt Reader", "Open local ALETHEIA receipt files as read-only review records without rescoring or certification."),
...
if selected_top_module == '🧾 Receipt Reader':
    with st.container():
        st.subheader("Receipt Reader")
        st.caption(
            "Read local ALETHEIA receipt files as review records. This utility does not rescore, certify, "
            "approve, reject, enforce, or override the original receipt."
        )
        render_shared_protocol_state_notice("Receipt Reader", expanded=False)
