...
Scope: Unit Preview visual-reference presentation and packaged local assets only. No scoring, routing, receipt schema, protocol logic, or authority boundary changed. Human review remains required.
...
Human review remains required. ALETHEIA surfaces signals; humans keep the judgment.
## Patch 136 Notes - Aletheia Unit Preview Stabilization

Current patch: Patch 136 - Aletheia Unit Preview Stabilization.

Patch 136 confirms that Aletheia Unit Preview remains a session-only suggestion layer and does not disturb routing, scoring, receipts, privacy posture, AI Integrity behavior, World Lens math, uploads/downloads, or module behavior. The normal app still renders after the gate passes.

Boundary preserved: test/check/docs only. No new UI feature, no new modules, no scoring, no verdict routing, no receipt schema, no receipt generation, no signal regex, no signal weights, no AI Integrity scan behavior change, no Privacy Audit scan behavior change, no World Lens math change, no external calls, no telemetry, no analytics, no tracking, no storage, no identity sync, no certification, no enforcement, no final-truth claim, and no privacy-guarantee claim. Human review remains required.

## Patch 135 Notes - Aletheia Unit Preview v1

Current patch: Patch 135 - Aletheia Unit Preview v1.

Patch 135 adds a small front-door preview before the full app appears. It lets users paste a short text, question, receipt, or scenario and shows a transparent suggested path such as Receipt Reader - Standard View, AI Integrity Mirror, Stress Test, Mirror Check / Question Review, Evidence Lab, World Lens, or Mirror Check.

The preview suggests where to begin; it does not decide. Users can still choose any module after entering ALETHEIA.
...
## Patch 142.4 Notes - Receipt Reader Narrative Standard View Output

Current patch: Patch 142.4 - Receipt Reader Narrative Standard View Output.
...
Patch 142.15 continues the Receipt Reader stabilization sequence after the parser, batch, World Lens bundle, and module-copy passes. The reader now behaves more like a verbal witness: it starts with a bounded “The mirror reflects...” brief, defines the native receipt state in plain language, shows metric observations as human-readable cards, keeps exact native values available behind a details expander, and frames repair prompts as Human-review questions.

Implementation notes:
- State definitions are display-only and do not alter taxonomy.
- Metric observations explain uploaded values without creating new values.
- Native receipt values remain available for audit/review.
- QUESTION_PROMPT remains not scored and metrics remain not applicable.
- World Lens remains an evidence-bundle reader, not a country certification or government rating.
...
Patch 146.1 follows Patch 146 by restoring the small GitHub repository link in Aletheia Unit Preview and adding the AI audit-loop evidence as local reviewer assets. The Unit Preview now includes a collapsed proof-of-concept card explaining that ALETHEIA can mirror external AI reviews, praise, self-descriptions, and critiques through the audit loop: external AI output -> ALETHEIA mirror reading -> human review.

Evidence available for review:

- Grok / xAI: capture, centralization, architectural opacity, missing verifiability, and service-misalignment pressure.
- Claude: evidence-boundary and mechanisms-vs-claims gaps after a repo/docs-based review.
