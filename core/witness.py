...
def _display_ai_static_scan_context_block(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"Primary protocol state: {_display_value(context.get('primary_protocol_state'))}",
        f"Primary protocol risk: {_display_value(context.get('primary_protocol_risk'))}",
        f"Primary protocol label: {_display_value(context.get('primary_protocol_label'))}",
