...
Patch 212 adds reviewable-input guidance to the Semantic Pressure Layer. Pressure codes now map to structural repair prompts that explain how to make a claim evidence-based, bounded, appealable, and repairable. The guidance is explicitly not flag-avoidance advice and does not encourage cosmetic rewriting.

Visible changes:
- Semantic pressure details now include a Reviewable Input Guidance table beside the pressure-code matrix.
- Receipt Reader semantic details now show the same guidance behind the pressure-code expander.
- Text reports generated by the semantic scanner include the guidance under pressure codes.

Boundary notes:
- Guidance/display only.
- No scoring, routing, MEI7 ethics gate, Z-axis mapping, Stress Test metrics, Evidence Lab calculations, World Lens math, receipt schema, storage, telemetry, certification, enforcement, or final-truth behavior changed.
- ALETHEIA rewards reviewable structure, not prettier language.

Validation target:

```bat
python -m py_compile app.py core\semantic_pressure_scanner.py ui\receipt_reader.py
python -c "from core.semantic_pressure_scanner import scan_semantic_pressure, reviewability_guidance_rows; s=scan_semantic_pressure('a group of bankers have world power in secret'); print(s.pressure_codes); print(reviewability_guidance_rows(s.pressure_codes))"
```
